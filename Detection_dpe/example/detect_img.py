...
def draw_labels(image, detections):
    for det in detections:
        ...
        cv2.putText(image, f'{conf}-{label}', (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
    return image
...
def process_images(input_folder, output_folder):
    ...
    for image_name in os.listdir(input_folder):
        ...
        with open(image_path, 'rb') as image_file:
            ...
            response = requests.post('http://localhost:8000/yolov8/detect/', files=files, data=payload)
...
if __name__ == '__main__':
    input_folder = 'pruebas/img'
    output_folder = 'pruebas/img'
